...
def run(random_seed , dictlist, data_dir, participant_csv, solar_data, load_data, lock):
    ...
    testname = '_75solar12_flat_17batt_'
    ...
    battery_capacity = 229.5
    central_battery = Central_Battery(cap_kWh=battery_capacity, cap_kW=battery_capacity, cycle_eff=0.9, ui_battery_discharge_windows_path=os.path.join(data_dir,"ui_battery_discharge_window_eg.csv"), lock=lock)
...
if __name__ == "__main__":
    ...
    with open(os.path.join(data_dir,participant_csv)) as f:
        ...
        for i in range(7):
            t = multiprocessing.Process(target=run, args=(i, participants, data_dir, participant_csv, solar_data, load_data, lock))
            t.start()
    # run()